 ...
 */
     public static BinaryTree<String> readBinaryTree1(BufferedReader bR)
        ...
        while (!nextToken.equals("()") && scan.hasNext()) {
            
            NodeInput nodeInput = new NodeInput(nextToken);
            
            // If invalid string, return null.
            if (!nodeInput.valid)
                return null;
            
            // If unable to insert, return null. (clashing node position)
            if (!insert(bt, nodeInput))
                return null;
     ...
     public static boolean insert(BinaryTree<String> bt, NodeInput nodeInput) {
         if (bt.root == null) {
             bt.root = new Node<>(null);
             bt.nIncompleteNodes = 1;
         }
         
         Node<String> current = bt.root;
         
         int nextDirection = nodeInput.nextDirection();
         ...
         while(nextDirection != 0) {
             ...
             nextDirection = nodeInput.nextDirection();
         }
         
         // end of path. now nextDirection == 0.
         if (current.data != null) // Check for clash.
             return false;
         
         bt.nIncompleteNodes--;
         current.data = nodeInput.label;
         
         if (bt.nIncompleteNodes < 0) System.out.println("ERROR");// Debug
         return true;
     }
     
     public boolean isComplete() {
         return nIncompleteNodes == 0;
     }
